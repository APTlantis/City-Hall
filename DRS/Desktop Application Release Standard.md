...
- [ ] Distribution and signing/provenance status are stated
...
Every release artifact requires a SHA-256 hash recorded in the release document. Publishable release artifacts should also carry an ARHS `.hashmanifest.toml` generated by ReleaseHasher or an equivalent documented command.

Minimum requirement: **SHA-256**, uppercase hex, no separators.

Preferred for security-sensitive and public releases: **SHA-256 in DRS records plus an ARHS manifest with SHA256, BLAKE3-256, and KT128**.
...
Signing:         [microsoft-store-signed / self-signed-development / ca-signed / unsigned-internal / other clear statement] on [Date]
```
...
| Distribution and signing/provenance | Explicit statement — never implicit |
...
| Release shape | Default distribution | Signing or provenance |
| --- | --- | --- |
...
Before building a Store submission package, reserve the product identity in Partner Center and record the Store-provided package identity fields. The Store candidate manifest must match those values exactly. A first submission may use Partner Center website upload instead of `msstore` automation when certification feedback is still being gathered.

For Microsoft Store packages, compute the ARHS hash manifest only after the final Store candidate package has been rebuilt and accepted by Partner Center package validation. Any identity, display-name, asset, or package-structure correction changes the MSIX/MSIXUPLOAD and requires fresh hash evidence.
...
- A public release omits the distribution channel or signing/provenance authority
- A sideloaded self-signed development build is described as a Microsoft Store or production-trusted release
